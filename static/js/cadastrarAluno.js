...
searchInput.addEventListener('keyup', function () {
    ...
    rows.forEach(row => {
        ...
        const turma = row.cells[0].textContent.toLowerCase();
        const genero = row.cells[2].textContent.toLowerCase();

        row.style.display = (nome.includes(filter) || matricula.includes(filter) || turma.includes(filter) || genero.includes(filter)) ? '' : 'none';
    });
});